...
// Centralized agent icon mapping
export const AGENT_ICONS: Record<string, React.ComponentType<IconProps>> = {
  'prd-generator': ClipboardIcon,
  'escopo-comercial': BriefcaseIcon,
  'escopo-tecnico': WrenchIcon,
};

export function AgentIcon({ slug, className }: { slug: string; className?: string }) {
  const Icon = AGENT_ICONS[slug] || BotIcon;
  return <Icon className={className} />;
}

// Centralized doc type icon mapping
export const DOC_TYPE_ICONS: Record<string, React.ComponentType<IconProps>> = {
  docx: FileTextIcon,
  pdf: FileIcon,
};

export function DocTypeIcon({ type, className }: { type: string; className?: string }) {
  const Icon = DOC_TYPE_ICONS[type] || FileIcon;
  return <Icon className={className} />;
}
